...
**GSB NOW** ใช้ template คนละแบบ จึงมี reader แยก `parseGsbTx()`:

- **ไม่มีหัวข้อ `เงินเข้า/ออก`** และ label ปลายทางเป็น `เข้าบัญชี` **ทั้งสองทิศ** (ไม่มี `ไปยังบัญชี`)
  ทิศทางจึงมาจาก 2 สัญญาณที่ต้องตรงกัน — ฝั่งไหนเป็นบัญชี GSB (`จากบัญชี` = เรา → `tx_out`)
  และเครื่องหมายของยอด (`-5,000.04 บาท` = ออก, `21.91 บาท` = เข้า)
- รายการ **GSB→GSB** ไม่มีสัญญาณฝั่ง → ใช้เครื่องหมายอย่างเดียว
- ถ้าสองสัญญาณขัดกัน หรือไม่มีฝั่งไหนเป็น GSB เลย → คืน `null` (ไม่เดา) แล้ว forward ตามปกติ
- เลขบัญชีมาเป็น `SCBA 0003XXXX9148` (bank code 4 ตัว + เลข mask กลาง) แยกด้วย `GSB_ACCOUNT`
  ไม่ใช่ `splitAccount()` ของ template อื่น

`knownBank()` ยังคืน `null` สำหรับ GSB (แม้ตอนนี้ parse ได้ทั้งสองทิศ) เพราะยังไม่มีตัวอย่าง
โปรโมชัน/rich-menu ของ GSB มายืนยันว่า drop แล้วปลอดภัย → `FILTER_EVENT` ยัง fail open
